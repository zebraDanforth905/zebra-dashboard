...
import { fetchTrialNotes } from '@/app/lib/data';
import { createTrialNote, deleteTrialNote } from '@/app/lib/actions';
import { TrialNote } from '@/app/lib/definitions';
...
export default function TrialNotesModal({ trialId, trialName, currentUserName, onClose }: Props) {
  const [notes, setNotes] = useState<TrialNote[]>([]);
  ...
  const loadNotes = async () => {
    try {
      const data = await fetchTrialNotes(trialId);
      setNotes(data);
    } catch (error) {
  ...
  const handleCreateNote = async (e: React.FormEvent) => {
    ...
    try {
      await createTrialNote(trialId, newNoteContent.trim(), currentUserName);
      setNewNoteContent('');
      await loadNotes();
    } catch (error) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 p-4" onClick={onClose}>
      <div className="relative w-full max-w-2xl rounded-lg bg-white p-6 shadow-xl max-h-[85vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
        <div className="mb-4 flex items-center justify-between border-b pb-4">
          <div>
            <h2 className="text-xl font-semibold text-gray-900">Trial Notes</h2>
        ...
        <div>
          ...
          {loading ? (
            ...
            <div className="py-8 text-center text-gray-500">No notes yet for this trial.</div>
          ) : (
