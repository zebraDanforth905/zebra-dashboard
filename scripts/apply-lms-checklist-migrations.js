...
// Applies (idempotent, safe to re-run): 033 app_settings, 041 create_user
// action, 042 sync_state rename, 040 PA day-camp assignments. Then reports
// the exact schema_ready gate the dashboard uses.
//
// Optional Canvas token seeding (check-first: only writes if no row exists):
//   CANVAS_SEED_TOKEN='<token>' node scripts/apply-lms-checklist-migrations.js
//
// Run in a shell where POSTGRES_URL points at the TARGET db (kyle-dev):
//   node scripts/apply-lms-checklist-migrations.js
const fs = require('fs');
